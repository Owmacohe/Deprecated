...
namespace Descant.Runtime
{
    public class DescantDialogueUI : MonoBehaviour
    {
        ...
        public void DisplayNode(int choiceIndex = 0)
        {
            ...
            if (dialogueController.Current.Data.Type.Equals("Choice") &&
                dialogueController.Current.Next[choiceIndex].Data.Type.Equals("Choice"))
            {
                response.text = "";   
            }
            ...
            else if (currentNodeType.Equals("Choice"))
            {
                foreach (var j in temp.Text)
                {
                    ...
                    tempChoice.GetComponentInChildren<EventTrigger>().triggers[2].callback.AddListener(_ =>
                    {
                        DisplayNode(copy); 
                    });
